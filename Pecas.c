	...
	int i,flag=0,x=(*a).posicao_x,y=(*a).posicao_y,tamanho_horizontal=x+(*a).tamanho;
		if((*a).orientacao==0){/*dependendo da orientacao da peca, agiremos diferentemente*/
			if(x==0 || x==25) return 1;/*primeiro verificamos se a peca ja 								esta nos extremos da tela. Nesse caso, ela
								nao pode mover*/
			for(i=0;i<(*a).tamanho;i++){
				if((*tela)[y+i][x+1].ocupado==1 || (*tela)[y+i][x-1].ocupado==1){/*aqui checamos se ha parte de outra peca ao lado da peca ativa. a flag sera usada para sair da funcao caso isso seja verdade*/
					flag=1;
				}
			}
		...
		if((*a).orientacao==1){
			...
			if((*tela)[y][x-1].ocupado==1 || (*tela)[y][x+tamanho_horizontal].ocupado==1) return 5;/*com pecas horizontais, so precisamos checar 2 posicoes*/
			if(direcao==1){
				((*a).posicao_x)++;
				return 0;
			}
